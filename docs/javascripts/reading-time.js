...
; (function (window) {
    ...
    function ReadingTime(numberWordsPerMinute, readingTimeLabel, minutesLabel, wordsLabel, lessThanAMinuteLabel) {
        ...
        const paragraphs = document.querySelectorAll('.detail-wrapper p');

        // The counter.
        let count = 0;

        for (let i = 0, parLen = paragraphs.length; i < parLen; i++) {
            // Split the innerHtml of the current paragraph to count the words.
            count += paragraphs[i].innerHTML.split(' ').length;
        }

        // Add 'Reading time:' label
        document.querySelector(".reading-time__label").innerText = readingTimeLabel;
        ...
        document.querySelector(".reading-time__word-count").innerText = `(${totalWords} ${wordsLabel})`;

        if (readingTimeDuration > 0) {
            if (readingTimeSeconds > 30) {
                readingTimeDuration = readingTimeDuration + 1
                document.querySelector(".reading-time__duration").innerText = `${readingTimeDuration} ${minutesLabel}`;
            }
            document.querySelector(".reading-time__duration").innerText = `${readingTimeDuration} ${minutesLabel}`;
        } else {
            document.querySelector(".reading-time__duration").innerText = lessThanAMinuteLabel;
        }
    }
