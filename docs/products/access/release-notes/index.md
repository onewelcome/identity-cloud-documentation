...
* Added a new event_type `TOKEN REQUEST EXPIRED REFRESH TOKEN`, which replaces the `TOKEN REQUEST INVALID REFRESH TOKEN` event when a refresh token is expired
