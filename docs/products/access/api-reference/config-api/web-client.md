...
| public_jwk                       | depends  | EC or RAS public key                | Plain public key for private key JWT authentication in the Elliptic Curve (min P-256) or RSA (min 2048, max 4096 key length) x509 format. The Onegini Access will favour `jwks_uri` if provided over statically defined JWK.              |
| jwks_uri                         | depends  | "https://authorization-server/jwks" | Uri of JWKS endpoint with public keys for private key JWT authentication.                                                                                                                                                   |
