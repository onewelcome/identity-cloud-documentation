...
| refresh_token_expires_in         | no       | 7200                               | Limits the time refresh tokens can be used to obtain a new access token and refresh token calculated from the initially issued access token. `refresh_token_enabled` must be `true` when providing a value.                                                     |
