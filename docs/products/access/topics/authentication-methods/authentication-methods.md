...
| `jti` | Optional     | String  | The unique token identifier. If you specify this parameter, the token can only be used once. As a result, subsequent token requests won't succeed. Is **Required** for [OIDC](https://openid.net/specs/openid-connect-core-1_0.html#ClientAuthentication) flows |
