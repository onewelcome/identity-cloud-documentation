...
| Redirect URL                         | yes      |                     | Location to which the end-user is redirected after successful authorization. This URI starts with the custom app scheme (e.g. myapp://) and this is the full URI to which the end-user is redirected after they have successfully logged in and given authorization for the requested scopes.
| Additional redirect URLs             | no       |                     | Additional URIs to which the end-user can be redirected. See "Redirect URL". Used to change the redirect URI for an already deployed application - the previously used redirect URL need to be moved as additional URL and the new one can be supplied as primary redirect URI. Mind that the `additional redirect URLs` are not exposed within application version export.
